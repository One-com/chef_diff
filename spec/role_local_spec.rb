...
describe ChefDiff::Changes::RoleLocal do
  let(:logger) do
    Logger.new('/dev/null')
  end
  let(:role_local_dir) do
    'roles_local'
  end
  ...
  fixtures = [
    ...
    {
      :name => 'add/modify a environment specific role',
      :files => [
        {
          :status => :modified,
        ...
        {
          :status => :modified,
          :path => 'roles_local/cluster/test.json'
        },
        {
          :status => :modified,
  ...
  fixtures.each do |fixture|
    it "should handle #{fixture[:name]}" do
      ChefDiff::Changes::RoleLocal.find(
        fixture[:files],
        role_local_dir,
        logger
      ).map do |cb|
